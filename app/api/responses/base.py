...
class BaseResponse:
    ...
    @staticmethod
    def success_response(status_code=status.HTTP_200_OK, message="Success"):
        """
    ...
    @staticmethod
    def error_response(status_code=status.HTTP_400_BAD_REQUEST, message="Error"):
        """
        The function `error_response` returns a JSON response with a success status code and message.

        :param status_code: The `status_code` parameter is used to specify the HTTP status code for the
        response. By default, it is set to `status.HTTP_400_BAD_REQUEST`, which indicates a error response.
